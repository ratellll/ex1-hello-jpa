...
권장하는 식별자 전략
- 기본키 제약 조건: null아님,유일,변하면안됌
- 미래까지 이 조건을 만족하는 자연키는 찾기 어려움 .
- 대리키를 사용해야함 (대체키) ex) 주민등록번호도 기본키로 적절하지않음
- 권장: Long형 + 대체키 + 키 생성전략 사용

IDENTITY 전략 특징
- 기본키 생성을 DB에 위임
- 커밋시점에 INSERT 실행을 하지만 IDENTITY 경우 예외적으로 persist가 들어갈때 insert 쿼리를 날려서 ID값을 확보한다

SEQUENCE 전략 특징
- IDENTITY와 다르게 커밋시점에 INSERT쿼리를 실행한다 persist가 들어갔을때 DB에서 다음 SEQUENCE를 불러온다
- allocationSize 이란 SEQUENCE를 생성할때마다 DB에 들려서 시퀀스를 불러오면 서버의 부담이될수있기때문에 미리 SEQUENCE를 다수 불러오는것이다
기본 키 매핑
...
객체의 양방향 관계
- 객체의 양방향 관계는 사실 양방향관계가 아니라 서로 다른 단방향관계 2개이다 
...
- 양방향 매핑은 반대방향으로 조회(객체 그래프 탐색) 기능이 추가된것뿐
- JPQL에서 역방향으로 탐색할 일이 많음
- 단방향 매핑을 잘하고 양방향은 필요할때 추가해도됨 (테이블에 영향을주지않음)
